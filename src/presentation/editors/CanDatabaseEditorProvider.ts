...
export class CanDatabaseEditorProvider implements vscode.CustomTextEditorProvider {
  ...
  static register(
    ...
    const provider = new CanDatabaseEditorProvider(context, databaseService, webviewMessageHandler);
  ...
  async resolveCustomTextEditor(
    ...
    try {
      ...
      Logger.error('resolveCustomTextEditor failed', error);
  ...
  private async loadDatabaseForEditor(document: vscode.TextDocument): Promise<void> {
    try {
      ...
      Logger.error('Failed to load database for editor', error);
      const msg = error instanceof Error ? error.message : String(error);
  ...
  private async buildHtml(webview: vscode.Webview): Promise<string> {
    ...
    try {
      ...
      try {
        ...
        Logger.warn(
          `Could not read webview from ${htmlDiskPath}, trying workspace.fs: ${diskErr instanceof Error ? diskErr.message : String(diskErr)}`,
        );
        const bytes = await vscode.workspace.fs.readFile(vscode.Uri.joinPath(distUri, 'index.html'));
      ...
      Logger.error('webview-ui/dist/index.html not found — run npm run build:webview', error);
