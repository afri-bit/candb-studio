...
export class CanDatabaseTreeProvider implements vscode.TreeDataProvider<BaseTreeItem> {
  ...
  ): { provider: CanDatabaseTreeProvider; treeView: vscode.TreeView<BaseTreeItem> } {
  ...
  private getRootItems(database: ReturnType<CanDatabaseService['getDatabase']> & {}): BaseTreeItem[] {
    ...
    const unlinked = database.signalPool.filter((s) => !database.isSignalReferencedByMessage(s.name));
