...
function signalValueTypeFromWebview(s: WebviewSignalInput): SignalValueType {
    if (s.valueType === 'float') {
        return SignalValueType.IEEEFloat;
    }
    if (s.valueType === 'double') {
        return SignalValueType.IEEEDouble;
    }
    return s.isSigned ? SignalValueType.Signed : SignalValueType.Unsigned;
}

function multiplexFromWebview(s: WebviewSignalInput): {
    indicator: MultiplexIndicator;
    value?: number;
} {
...
export function patchAttributeDefinition(
    ...
    if ('minimum' in changes && typeof changes.minimum === 'number') {
        def.minimum = changes.minimum;
    }
    if ('maximum' in changes && typeof changes.maximum === 'number') {
        def.maximum = changes.maximum;
    }
