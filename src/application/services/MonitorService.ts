...
export class MonitorService implements ICanBusMonitor {
  ...
  private pendingTransmits: Array<{ id: number; isExtended: boolean; data: Uint8Array; t: number }> = [];
  ...
  private classifyDirection(frame: CanFrame): MonitorFrameDirection {
    const now = Date.now();
    this.pendingTransmits = this.pendingTransmits.filter((p) => now - p.t < PENDING_TX_MAX_AGE_MS);
