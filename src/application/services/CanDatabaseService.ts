...
export class CanDatabaseService {
   ...
   */
  updateSignal(uri: string, messageId: number, signalName: string, changes: Record<string, unknown>): void {
    const db = this.requireDatabase(uri);
    const msg = db.findMessageById(messageId);
    if (!msg) {
      throw new Error(`Message id ${messageId} not found`);
  ...
  updateValueTable(uri: string, name: string, changes: Record<string, unknown>): void {
    ...
    if ('entries' in changes && changes.entries !== null && typeof changes.entries === 'object') {
      vt.entries.clear();
  ...
  private applySignalChanges(signal: Signal, changes: Record<string, unknown>): void {
    ...
    if ('valueDescriptions' in changes && changes.valueDescriptions !== null && typeof changes.valueDescriptions === 'object') {
      const vd = new Map<number, string>();
      Object.entries(changes.valueDescriptions as Record<string, string>).forEach(([k, val]) => {
        vd.set(Number(k), val);
      });
