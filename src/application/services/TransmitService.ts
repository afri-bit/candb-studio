...
export class TransmitService implements ICanBusTransmitter {
    private tasks = new Map<
        string,
        { task: TransmitTask; timer: ReturnType<typeof setInterval> }
    >();
    ...
    startPeriodic(task: TransmitTask): void {
        ...
        const timer = setInterval(async () => {
            try {
                ...
                Logger.error(`Transmit: periodic send failed for task ${task.id}`, err);
                this.stopPeriodic(task.id);
            }
        }, task.intervalMs);
    ...
    updatePeriodicInterval(messageId: number, intervalMs: number): boolean {
        ...
        const timer = setInterval(async () => {
            try {
                ...
                Logger.error(`Transmit: periodic send failed for task ${taskId}`, err);
